"""
This file defines a useful high-level abstraction to build Gradio chatbots with a different layout: MvpInterface.
"""
...
@document()
class MvpInterface(Blocks):
    """
    MvpInterface is JD's high-level abstraction for creating chatbot UIs, and allows you to create
    ...
    Example:
        ...
        demo = gr.MvpInterface(fn=echo, examples=["hello", "hola", "merhaba"], title="Echo Bot")
        demo.launch()
    Demos: mvpinterface_random_response, mvpinterface_streaming_echo
        ...
        with self:
            if title or image or description:
                with Row(equal_height=False):
                    if image:
                        with Column(scale=1, min_width= 12):
                            Image(image, width=10, show_label=False, show_download_button=False)
                    with Column(scale=8):
                        if title:
                            with Row():
                                Markdown(
                                    f"<h1 style='text-align: left'>{self.title}</h1>"
                                )
                        if description:
                            with Row():
                                Markdown(
                                    f"<h3 style='text-align: left; margin-bottom: 1rem'>{description}</h3>"
                                )
            ...
            with Column(variant="panel"):
                with Row(equal_height=False):
                    if chatbot:
                        ...
                        if like is not None:
                            self.chatbot.like(like, None, None)
                    else:
                ...
                # The order here is really important, so reorder based on the order the above buttons are added to the self.buttons list or behavior breaks.
                (
                    self.retry_btn,
                    self.undo_btn,
                    self.clear_btn,
